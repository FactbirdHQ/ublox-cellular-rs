    ...
    async fn handle_urc(&mut self, event: Urc) {
        match event {
            ...
            Urc::SocketClosed(socket) => {
                ...
                // implementation would use a SocketSet registry to track per-socket state,
                // and only set LinkState::Down when the underlying data connectivity is lost
                // (e.g., PDP context deactivation, network deregistration), not when individual
